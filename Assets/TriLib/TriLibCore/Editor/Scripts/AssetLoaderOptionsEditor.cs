        ...
        public static void ShowInspectorGUI(SerializedObject serializedObject, ref int currentTab)
        {
            ...
            switch (currentTab)
            {
                case 0:
                    ...
                    EditorGUILayout.PropertyField(disableObjectsRenamingProperty, new GUIContent("Disable Objects Renaming", "Turn on this field to disable objects renaming."));
                    if (!disableObjectsRenamingProperty.boolValue)
                    {
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("MaxObjectsToRename"), new GUIContent("Max Objects to Rename", " Use this field to set the maximum number of objects TriLib can rename. Renaming is an expensive process, so it's advised to keep a low value."));
                    }
                    ...
                    if (importMeshesProperty.boolValue)
                    {
                        ...
                        if (importBlendShapesProperty.boolValue)
                        {
                            ...
                            EditorGUILayout.PropertyField(serializedObject.FindProperty("BlendShapeMapper"), new GUIContent("Blend Shape Mapper", "Use this field to set a BlendShapeMapper to use with the loaded model. BlendShapeMappers can replace the Unity built-in blend shape playback system."));
                        }
                        ...
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("UseSharedMeshes"), new GUIContent("Use Shared Meshes", "Turn off this field to use mesh filters \"mesh\" property instead of \"sharedMesh\" property."));
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("MeshWorldTransform"), new GUIContent("Mesh World Transform", "Use this field to define a transformation applied in world space to all mesh vertices."));
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("UpdateSkinnedMeshRendererWhenOffscreen"), new GUIContent("Update Skinned Mesh Renderer when Ofscreen", "Turn on this field to update SkinnedMeshRenderers when they're offscreen."));
                    }
                            ...
                            switch (avatarDefinitionType)
                            {
                                case AvatarDefinitionType.CreateFromThisModel:
                                    if (animationType == AnimationType.Humanoid)
                                    {
                                        ...
                                        EditorGUILayout.PropertyField(serializedObject.FindProperty("ApplyAvatarHipsCompensation"), new GUIContent("Apply Avatar Hips Compensation", "Turn off this field if your avatars seem to hover over the ground. That will disable avatar hips height compensation."));
                                    }
                                    break;
                ...
                case 3:
                    ...
                    if (importMaterialsProperty.boolValue)
                    {
                        ...
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("UseSharedMaterials"), new GUIContent("Use Shared Materials", "Turn on this field to use renderers \"sharedMaterials\" property instead of \"materials\" property."));
                    }
                    break;
                case 4:
                    ...
                    if (importTexturesProperty.boolValue)
                    {
                        ...
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("LoadTexturesViaWebRequest"), new GUIContent("Load Textures via Web Request", "Turn on this field to load textures using the UnityWebRequest class (experimental). UnityWebRequest is the fastest way to load PNG/JPG textures but uses more memory than the other methods."));
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("ConvertTexturesAs2D"), new GUIContent("Convert Textures as 2D", "Turn off this field to keep processed/composed textures as RenderTextures."));
                    }
                    break;
                case 5:
