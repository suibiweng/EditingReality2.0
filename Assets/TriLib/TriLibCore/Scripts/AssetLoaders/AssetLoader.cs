...
#endif
            if ((GraphicsSettingsUtils.IsUsingUniversalPipeline || GraphicsSettingsUtils.IsUsingHDRPPipeline) && assetLoaderOptions.LoadTexturesAsSRGB)
            {
                Debug.LogWarning("Textures must be loaded as Linear on the UniversalRP and HDRP.");
                Debug.LogWarning(ValidationMessage);
        ...
        private static IEnumerable CreateGeometry(AssetLoaderContext assetLoaderContext, GameObject meshGameObject, IRootModel rootModel, IModel meshModel)
        {
            ...
            if (geometryGroup.GeometriesData != null)
            {
                ...
                if (assetLoaderContext.Options.AnimationType != AnimationType.None || assetLoaderContext.Options.ImportBlendShapes)
                {
                    ...
                    if ((bones != null && bones.Count > 0 || assetLoaderContext.Options.BlendShapeMapper == null && geometryGroupBlendShapeGeometryBindings != null && geometryGroupBlendShapeGeometryBindings.Count > 0) && assetLoaderContext.Options.AnimationType != AnimationType.None)
                    {
                ...
                if (renderer != null)
                {
                    ...
                    foreach (var geometryData in geometryGroup.GeometriesData)
                    {
                        ...
                        if (hasInvalidMaterial)
                        {
                            ...
                            materialRendererContext.Mesh = geometryGroup.Mesh;
                            materialRendererContext.Context = assetLoaderContext;
        ...
        private static void ApplyMaterialToRenderers(MaterialMapperContext materialMapperContext)
        {
            ...
            if (materialMapperContext.Context.MaterialRenderers.TryGetValue(materialMapperContext.Material, out var materialRendererList))
            {
                for (var i = 0; i < materialRendererList.Count; i++)
                {
                    ...
                    materialRendererContext.Mesh = meshFilter != null ? meshFilter.sharedMesh : skinnedMeshRenderer != null ? skinnedMeshRenderer.sharedMesh : null;
